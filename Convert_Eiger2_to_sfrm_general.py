...
def init_argparser():
    parser = argparse.ArgumentParser(description = 'Convert Eiger2 .h5 data to Bruker .sfrm')
    parser.add_argument('-f', required=True,  default='',           metavar='path',  type=str,   dest='_H5F', help='h5 master file [required]')
    parser.add_argument('-p', required=False, default='entry/data', metavar='path',  type=str,   dest='_H5P', help='h5 path to data [required]')
    ...
    parser.add_argument('-k', required=False, default=0,            metavar='int',   type=int,   dest='_ROT', help='horizontal [0] or vertical [1] goniometer')
...
def prepare_saint_mask(set, _ARGS):
    ...
    with h5py.File(_ARGS._H5F, 'r') as h5:
        data = h5[_ARGS._H5P][set][0,:,:]
    
    # The goniometer is vertical, rotate by 90
    if _ARGS._ROT > 0:
        data = np.rot90(data, k=_ARGS._ROT, axes=(1, 0))
    ...
    header['DETTYPE'][:] = ['Eiger2', pix_per_512, 0.00, 0, 0.001, 0.0, 0]
...
def convert_Eiger2_Bruker(fname, inum, isum, iexp, iosr, idat, irot,
                          src_wav=0.245479, gon_chi=0.0, gon_phi=0.0, gon_tth=0.0, gon_dxt=103.5124,
                          det_max=65535, det_bcx=508.6675, det_bcy=469.8171):
    
    # Goni is vertical, rotate by 90
    if irot > 0:
        idat = np.rot90(idat, k=irot, axes=(1, 0))
        # Adjust beam center x/y to the rotation
        if irot == 1:
            det_bcx, det_bcy = det_bcy, idat.shape[0] - det_bcx
        elif irot == 2:
            det_bcx, det_bcy = idat.shape[1] - det_bcx, idat.shape[0] - det_bcy
        elif irot == 3:
            det_bcx, det_bcy = idat.shape[1] - det_bcy, det_bcx
        else:
            print('Unreasonable detector rotation!')
            return False
    ...
    # this is normalized to a 512x512 detector format
    # Eiger2-1M pixel size is 0.075 mm
    pix_per_512 = round((10.0 / 0.075) * (512.0 / idat.shape[0]), 6)
...
def parallel_read(set, idx, inum, _ARGS, overwrite=True):
    # guess a name
    ...
    with h5py.File(_ARGS._H5F, 'r') as h5:
        arr2d = np.sum(h5[_ARGS._H5P][set][idx:idx+_ARGS._SUM,:,:], axis=0)
    # convert the summed 2d array to Bruker sfrm
    # if successful returns True else False
    return convert_Eiger2_Bruker(inam, inum, _ARGS._SUM, _ARGS._FET, _ARGS._OSR, arr2d, _ARGS._ROT, src_wav=_ARGS._WAV, det_max=_ARGS._IBD*_ARGS._SUM, gon_chi=_ARGS._CHI, gon_tth=_ARGS._TTH)
    
if __name__ == '__main__':
    '''
     Script to convert Eiger2 data to Bruker .sfrm format
     ...
     Specify the goniometer orientation horizontal/vertical
      -k: 0, default, horizontal orientation,  omega axis looking up
          1, vertical orientation (right in the direction of the beam)
          2, horizontal orientation hanging, looking down to the beam
          3, vertical orientation flipped, left to the beam

     The wavelength stored if the h5 file might be inaccuarate as it is derived
     from the energy threshhold and therefore has to be provided as well
      -w: Wavelength
     
     Images can be combined to reduce the number of sfrm files
      -s: Number of images to sum
          Exposure time and scan range are adjusted accordingly
    ...
    with h5py.File(_ARGS._H5F, 'r') as h5f:
        # frame exposure time
        _ARGS._FET = h5f.visit(lambda key: round(h5f[key][()], 6) if isinstance(h5f[key], h5py.Dataset) and h5f[key].name.endswith('frame_time') else None)
        # image bit depth
        _ibd = h5f.visit(lambda key: h5f[key][()] if isinstance(h5f[key], h5py.Dataset) and h5f[key].name.endswith('bit_depth_image') else None)
        _ARGS._IBD  = 2**_ibd-1
        # beam center x/y
        _ARGS._BCX = h5f.visit(lambda key: h5f[key][()] if isinstance(h5f[key], h5py.Dataset) and h5f[key].name.endswith('x_pixels_in_detector') else None)
        _ARGS._BCY = h5f.visit(lambda key: h5f[key][()] if isinstance(h5f[key], h5py.Dataset) and h5f[key].name.endswith('y_pixels_in_detector') else None)
        # the h5 files we need to iterate
        h5files = list(h5f[_ARGS._H5P].keys())
        # check if the files are available or None
        h5data = [h5f[_ARGS._H5P].get(i, default=None) for i in h5files]
    ...
    with mp.Pool() as pool:
        ...
        for ih5, entry in enumerate(h5files):
            ...
            while idx + _ARGS._SUM <= h5inum[ih5]:
                ...
                #parallel_read(entry,idx,inum,_ARGS)
                # increment idx
                idx += _ARGS._SUM
